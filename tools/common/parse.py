...
def split(b):
    ...
    У части писаний строфа и её транслитерация стоят **в одном абзаце**: сперва
    строки деванагари, под ними те же строки латиницей. У других они лежат
    двумя стенами — сперва весь санскрит раздела, потом вся транслитерация, — и
    `pairing` в common/page.py складывает стены построчно. Чтобы страница
    выходила одна и та же, абзац здесь разводится надвое: тот же `pairing`
    собирает его обратно уже построчно, а не двумя блоками подряд.
    ...
    Номер строфы отсюда не берётся: у Śivastotrāvalī он стоит в конце строфы
    («||13.11||»), у Mālinīvijayottaratantra то же место занимает то номер
    строфы, то номер главы в колофоне. Правило это у каждого писания своё, и
    живёт оно в его `convert.py`.
    """
