...
def cut(span):
    ...
    # Основа, кончающаяся знаком разметки, подстрочником быть не может: `_`
    # закрывает курсив, открытый снаружи, и внутри подстрочника тот остался бы
    # незакрытым. Переставить знак наружу нельзя: со снятыми тегами строка
    # обязана совпасть с прежней знак в знак, а перестановка её меняет. Такое
    # остаётся скобкой в строке.
    tail = span[edge:].rstrip()
    if tail and tail[-1] in '_*':
        return span, ''
...
def markup(t, link=None):
    ...
    такое уточнение начинается в одном абзаце, а кончается в следующем, и
    тогда в абзаце дефисов нечётное число. Пары в нём поедут на одну: закроется
...
def notes(t, open='<span class="pv-note">— ', close=' —</span>'):
    if t.count('--') % 2:
        return t
    return re.sub(r'--(.+?)--', lambda m: open + m.group(1) + close, t, flags=re.S)
...
def italic(t):
    """`_…_` → тег, а не подчёркивания на странице.

    Курсивом у Габриэля набрано то, чего в подлиннике нет, а он дописал ради
    связности: `_(This)_`. Правило то же, что у `bold`, и по той же причине:
    абзацу, в котором стоит `||63||`, разметку kramdown доверить нельзя — он
    прочтёт вертикальные черты как таблицу.

    Одиночное подчёркивание оставляется текстом: пара найдётся где-нибудь
    далеко, и курсивом уйдёт пол-абзаца. Как и с двойными дефисами в `notes`,
    нечётное число значит «здесь разметки нет».

    Через тег пара не ставится. К этому времени в строке уже стоят подстрочник
    и уточнения автора, и курсив, начатый внутри `<span class="pv-note">` и
    закрытый снаружи, дал бы перехлёст тегов: счёт открытых и закрытых сойдётся,
    а разбор страницы поедет — у Габриэля курсив внутри двойных дефисов есть.
    Поэтому между парой не может стоять ни `<`, ни `>`, ни третье подчёркивание.
    """
    if t.count('_') % 2:
        return t
    return re.sub(r'(?<!\w)_([^<>_]+?)_(?!\w)',
                  lambda m: '<em>%s</em>' % m.group(1).replace('\n', '</em>\n<em>'),
                  t, flags=re.S)


def emphasis(t):
    """Полужирный и курсив источника там, где markdown до текста не доходит."""
    return italic(bold(t))
...
class Book:
    ...
    def todo(self, pid, n):
        """Предупреждение вверху страницы: сколько абзацев без перевода.

        `pid` нужен там, где непереведённое бывает разного рода: у
        Mālinīvijayottaratantra на одной странице это английские абзацы, а на
        соседней — строфы, к которым у источника нет и английского.
        """
...
def render(book, pid, slug, name, idx):
    ...
    for i, b in enumerate(blocks):
        ...
        if k == 'rule':
            ...
            if v is None:
                ...
                # Полужирный и курсив здесь ставятся руками: `markdown="1"`
                # этому абзацу не поставить — у источника в нём стоит «||63||»,
                # и kramdown прочтёт черты как таблицу.
                body.append('<p class="%s"%s lang="en">%s</p>'
                            % (mark('pv-en', a), ident(a),
                               emphasis(markup(b['t'])).replace('\n', '<br />\n')))
            else:
    ...
    missing_note = book.todo(pid, len(missing)) if missing else ''
